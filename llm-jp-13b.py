...
for model, file_path in file_paths.items():
    ...
    df['tokens'] = df['iteration'] * 1536 * 2048  # Convert iterations to tokens
    dataframes[model] = df
...
token_ticks = np.arange(0, max(df['tokens'].max() for df in dataframes.values()), 50e9)  # 20e9 tokens = 20B tokens
